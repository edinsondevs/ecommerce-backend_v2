...
describe('--------------------          ----------------------------', () => {
	describe('✅🎉 MÓDULO DE PRODUCTOS: CASOS EXITOSOS 🎉✅', () => {
		...
		it('6. Debería eliminar un producto correctamente', async () => {
			...
			const body = JSON.parse(response.payload);
			expect(body.data.id).toBe(productId); // Verificamos que nos devolvió el producto eliminado
			expect(body.message).toBe('Producto enviado a la papelera (Soft Delete)'); // Verificamos que el mensaje sea el correcto para eliminación lógica
			// 5. IMPORTANTE: Limpiamos el espía para que no afecte a otros tests
			mockDelete.mockRestore();
		});
...
describe('⚙️ Unit Tests: Product Service', () => {
	it('Debería actualizar un producto en la base de datos (Cobertura update)', async () => {
	...
	it('Debería buscar productos filtrados por stock (Cobertura findByStock)', async () => {
...
it('Debería hacer un borrado lógico (soft delete) si isDeleteLogic es true', async () => {
